<?php
	...
	*/
	...
	// custom menu
	if( !function_exists('attorna_get_custom_menu') ){
		function attorna_get_custom_menu( $settings = array() ){
			if( !empty($settings['type']) ){
				if( $settings['type'] == 'overlay' ){
					attorna_get_overlay_menu($settings);
				}else if( $settings['type'] == 'left' || $settings['type'] == 'right' ){
					$settings['slide'] = $settings['type'];
					attorna_get_mmenu($settings);
				}
			}
		}
	}
	...
	// menu icon
	if( !function_exists('attorna_get_mobile_menu_icon') ){
		function attorna_get_mobile_menu_icon( $settings = array() ){

			$settings = wp_parse_args($settings, array(
				'href' => '#',
				'button-type' => attorna_get_option('general', 'right-menu-style', 'hamburger-with-border'),
 				'button-class' => '',
				'icon-class' => 'icon_menu'
			));

			$button_class  = $settings['button-class'];
			$button_class .= ' attorna-mobile-button-' . $settings['button-type'];
	...
	// overlay menu
	if( !function_exists('attorna_get_overlay_menu') ){
		function attorna_get_overlay_menu( $settings = array() ){
			...
			echo '<div class="attorna-overlay-menu ' . esc_attr($settings['container-class']) . '" id="' . esc_attr($settings['id']) . '" >';
			
			$settings['button-class'] = 'attorna-overlay-menu-icon ' . $settings['button-class'];
			attorna_get_mobile_menu_icon($settings);

			echo '<div class="attorna-overlay-menu-content attorna-navigation-font" >';
			echo '<div class="attorna-overlay-menu-close" ></div>';

			echo '<div class="attorna-overlay-menu-row" >';
			echo '<div class="attorna-overlay-menu-cell" >';
			wp_nav_menu(array(
	...
	// mmenu
	if( !function_exists('attorna_get_mmenu') ){
		function attorna_get_mmenu( $settings = array() ){
			...
			$settings['button-class'] = 'attorna-mm-menu-button ' . $settings['button-class'];
			$settings['href'] = '#' .  $settings['id'];
			attorna_get_mobile_menu_icon($settings);

			echo '<div class="attorna-mm-menu-wrap attorna-navigation-font" id="' . esc_attr($settings['id']) . '" data-slide="' . esc_attr($settings['slide']) . '" >';
			wp_nav_menu(array(
				'theme_location'=>$settings['theme-location'], 
				'container'=> '', 
				'menu_class'=> 'm-menu'
	...
	if( class_exists('gdlr_core_edit_nav_menu') ){
		new gdlr_core_edit_nav_menu(array(
			'icon-class' => array(
				'title' => esc_html__('Menu Icon Class', 'attorna'),
				'type' => 'text',
			),
			'enable-mega-menu' => array(
				'title' => esc_html__('Enable Mega Menu', 'attorna'),
				'type' => 'checkbox',
				'depth' => '0'
			),
			'mega-menu-background' => array(
				'title' => esc_html__('Mega Menu Background', 'attorna'),
				'type' => 'upload',
				'depth' => '0'
			),
			'mega-menu-background-position' => array(
				'title' => esc_html__('Mega Menu Background Position', 'attorna'),
				'type' => 'combobox',
				'options' => array(
					'center' => esc_html__('Center', 'attorna'),
					'top-left' => esc_html__('Top Left', 'attorna'),
					'top-center' => esc_html__('Top Center', 'attorna'),
					'top-right' => esc_html__('Top Right', 'attorna'),
					'center-left' => esc_html__('Center Left', 'attorna'),
					'center-right' => esc_html__('Center Right', 'attorna'),
					'bottom-left' => esc_html__('Bottom Left', 'attorna'),
					'bottom-center' => esc_html__('Bottom Center', 'attorna'),
					'bottom-right' => esc_html__('Bottom Right', 'attorna'),
				),
				'depth' => '0'
			),
			'mega-menu-background-repeat' => array(
				'title' => esc_html__('Mega Menu Background Repeat', 'attorna'),
				'type' => 'combobox',
				'options' => array(
					'cover' => esc_html__('Cover ( full width and height )', 'attorna'),
					'no-repeat' => esc_html__('No Repeat', 'attorna'),
					'repeat' => esc_html__('Repeat X & Y', 'attorna'),
					'repeat-x' => esc_html__('Repeat X', 'attorna'),
					'repeat-y' => esc_html__('Repeat Y', 'attorna'),
				),
				'default' => 'cover',
				'condition' => array( 'background-type' => 'image' )
			),
			'mega-menu-width' => array(
				'title' => esc_html__('Mega Menu Width ( Fill value with % or px )', 'attorna'),
				'type' => 'text',
				'default' => '100%',
				'depth' => '0'
			),
			'hide-menu-title' => array(
				'title' => esc_html__('Hide Menu Title', 'attorna'),
				'type' => 'checkbox',
				'depth' => '1'
			),
			'mega-menu-section-size' => array(
				'title' => esc_html__('Section Size ( Only for mega menu )', 'attorna'),
				'type' => 'combobox',
				'options' => array( 
			...
			'mega-menu-section-content' => array(
				'title' => esc_html__('Section Content ( Only for mega menu )', 'attorna'),
				'type' => 'textarea',
				'depth' => '1'
			),
		));
	...
	if( !class_exists('attorna_menu_walker') ){
		
		// from wp-includes/nav-menu-template.php file
		class attorna_menu_walker extends Walker_Nav_Menu{
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				if( $depth == 0 ){
					...
					if( ceil($this->top_level_items / 2) + 1 == $this->top_level_count ){
						$center_nav_item = apply_filters('attorna_center_menu_item', '');
						if( !empty($center_nav_item) ){
							$output .= '<li class="attorna-center-nav-menu-item" >' . $center_nav_item . '</li>';
						}
					}
				}
				...
				if( $depth == 0 ){
					if( $item->gdlr_core_nav_menu_custom['enable-mega-menu'] == 'disable' ){
						$class_names .= ' attorna-normal-menu';
					}else{
						$class_names .= ' attorna-mega-menu';
					}
				}else if( $depth == 1 ){
				...
				if( $depth == 1 && !empty($item->gdlr_core_nav_menu_custom['mega-menu-section-content']) ){
					$item_output .= '<div class="attorna-mega-menu-section-content">';
