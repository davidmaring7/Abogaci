...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="attorna-single-article attorna-blog-quote-format" >
	<?php 
		// post title
		...
		if( !empty($thumbnail_id) ){
			...
			echo '<div class="attorna-blog-quote-background" ' . gdlr_core_esc_style(array(
				'background-image' => $quote_background
			)) . ' ></div>';
		}

		echo '<div class="attorna-blog-quote gdlr-core-quote-font" >&#8220;</div>';
		echo '<div class="attorna-blog-content-wrap" >';
		if( !empty($blockquote) ){
			echo '<div class="attorna-blog-quote-content gdlr-core-info-font">' . gdlr_core_content_filter($blockquote, true) . '</div>';
		}
		if( !empty($author) ){
			echo '<div class="attorna-blog-quote-author gdlr-core-info-font" >' . gdlr_core_escape_content(gdlr_core_text_filter($author)) . '</div>';
		}
		echo '</div>';
		echo '</div>';
	?>
