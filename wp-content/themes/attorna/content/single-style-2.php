<?php
	...
	echo '<div class="attorna-single-author-tags clearfix" >';
	...
	if( attorna_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				'<span class="attorna-single-nav-area-left" >%link</span>',
				'<span class="attorna-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($prev_post->ID))) . ' ></span>' . 
				'<span class="attorna-single-nav-area-overlay" ></span>' . 
				'<span class="attorna-single-nav attorna-single-nav-left">' . 
				'<i class="arrow_left" ></i>' . 
				'<span class="attorna-text" >' . esc_html__( 'Previous Post', 'attorna' ) . '</span>' .
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link(
				'<span class="attorna-single-nav-area-right" >%link</span>',
				'<span class="attorna-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($next_post->ID))) . ' ></span>' .
				'<span class="attorna-single-nav-area-overlay" ></span>' .
				'<span class="attorna-single-nav attorna-single-nav-right">' . 
				'<span class="attorna-text" >' . esc_html__( 'Next Post', 'attorna' ) . '</span>' . 
		...
		if( !empty($prev_post_html) || !empty($next_post_html) ){
			echo '<div class="attorna-single-nav-area clearfix" >' . $prev_post_html . $next_post_html . '</div>';
		}
	}
	...
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		...
		if( $query->have_posts() ){
			...
			echo '<div class="attorna-single-related-post-wrap" >';
			echo '<div class="attorna-single-related-post-container" >';
			echo '<div class="attorna-single-related-post-content attorna-item-rvpdlr" >';
			echo '<h3 class="attorna-single-related-post-title attorna-item-pdlr" >' . esc_html__('Related Posts', 'attorna') . '</h3>';
			...
			echo '</div>'; // attorna-single-related-post-content
			echo '</div>'; // attorna-single-related-post-container
	...
	if( comments_open() || get_comments_number() ){
		...
		echo '<div class="attorna-comment-container attorna-container">';
		echo '<div class="attorna-comment-content attorna-item-pdlr">';
		comments_template();
		echo '</div>';
		echo '</div>';
		echo '</div>';
